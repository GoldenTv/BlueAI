...
// .wrangler/tmp/bundle-DqJdaI/checked-fetch.js
...
// .wrangler/tmp/bundle-DqJdaI/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-DqJdaI/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-DqJdaI/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
// src/index.ts
var SYSTEM_PROMPT = "You are BlueAI, a helpful, intelligent, and friendly AI assistant. Always provide clear and well-structured answers in Thai or English as requested. Use Markdown formatting and LaTeX for all mathematical formulas: use $...$ or \\(...\\) for inline math, and $$...$$ or \\[...\\] for block math.";
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (!apiKey) {
        return new Response(
          JSON.stringify({
            error: "AI_API_KEY is not configured. Set it using 'npx wrangler secret put AI_API_KEY'."
          }),
          {
            status: 500,
            headers: {
      ...
      const baseURL = (env.AI_BASE_URL || "https://ai.psu.blue/v1").trim().replace(/\/$/, "");
      const customFetch = /* @__PURE__ */ __name(async (input, init) => {
        ...
        const transformedStream = new ReadableStream({
          async pull(controller) {
            while (true) {
              ...
              if (done) {
                if (inReasoning) {
                  controller.enqueue(encoder2.encode('data: {"choices":[{"delta":{"content":"</think>"}}]}\n\n'));
                  inReasoning = false;
                }
                controller.close();
                return;
                    ...
                    try {
                      ...
                      if (delta) {
                        ...
                        if (reasoningContent != null && reasoningContent.length > 0) {
                          if (!inReasoning) {
                            inReasoning = true;
                            delta.content = "<think>" + reasoningContent;
                          } else {
...
`));
                          continue;
                        } else if (normalContent != null && inReasoning) {
                          inReasoning = false;
                          delta.content = "</think>" + normalContent;
                          controller.enqueue(encoder2.encode(`data: ${JSON.stringify(parsed)}
`));
                          continue;
                        }
      ...
      const modelName = typeof body?.model === "string" && body.model.trim().length > 0 ? body.model.trim() : (env.AI_MODEL || "openai/gpt-5.6-luna").trim();
        ...
        async start(controller) {
          try {
            for await (const part of result.fullStream) {
              if (part.type === "reasoning") {
                const data = JSON.stringify({ type: "thinking", delta: part.textDelta });
                controller.enqueue(encoder.encode(`data: ${data}

`));
              } else if (part.type === "text-delta") {
                const data = JSON.stringify({ type: "text", delta: part.textDelta });
                controller.enqueue(encoder.encode(`data: ${data}

`));
              } else if (part.type === "error") {
                const data = JSON.stringify({ type: "error", error: String(part.error) });
                controller.enqueue(encoder.encode(`data: ${data}
...
`));
            ...
            const data = JSON.stringify({ type: "error", error: streamErr?.message || String(streamErr) });
            controller.enqueue(encoder.encode(`data: ${data}
